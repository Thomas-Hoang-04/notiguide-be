...
class DeviceBootstrapListener(
    ...
    private val topicFilter by lazy { "${mqttProperties.topicPrefix}/receiver/bootstrap/+" }
    private val registerTopic by lazy { "${mqttProperties.topicPrefix}/receiver/bootstrap/register" }
            ...
            scope.launch {
                runCatching {
                    deviceRegistrationService.onRegister(
                        ...
                        family = DeviceFamily.RECEIVER
                    )
                }.onFailure { ex ->
                    log.warn("Receiver bootstrap registration handling failed", ex)
                }
            }
